...
def text_to_embedding(
    ...
    for i in range(0, len(text), batch_size):
        ...
        output_embs.append(
            process_batch(batch_slice, {flag: encoder}, normalize_embeddings, device)[
                flag
            ]
        )
...
def text_batch(
    ...
    for target_flag, inverter in inverters.items():
        ...
        gt_emb = text_to_embedding(
            gt,
            target_flag,
            encoders[target_flag],
            normalize_embeddings,
            max_seq_length,
            device,
        )
        ...
        for key in list(d.keys()):
            ...
            if isinstance(val, dict):
                ...
                if ses and "var" in key:
                    d[f"{key.replace('var', 'se')}"] = np.sqrt(d[key]) / np.sqrt(
                        len(val) * bs
                    )
            else:
                d[key] = val
...
def top_k_accuracy(sims, k=1):
    ...
    return np.mean(
        np.any(top_k_preds == correct, axis=1)
...
def create_heatmap(
    translator, ins, tgt_emb, src_emb, top_k_size, heatmap_size=None, k=16
) -> dict:
    res = {}
    ...
    res[f"{src_emb}_{tgt_emb}_top_1_acc"] = (
        sims.argmax(axis=1) == np.arange(sims.shape[0])
    ).mean()
...
def compute_topk_accuracy_and_avg_rank(
    sorted_indices: torch.Tensor, ground_truth: torch.Tensor, k: int
):
    """
...
def top_k_p_r(predictions: np.ndarray, ground_truth: np.ndarray, k: int):
    ...
    precision_k = precision_score(
        ground_truth, pred_one_hot, average="samples", zero_division=0
    )
...
def classification_batch(ins, translations, labels, gt_mapping, k=1):
    ...
    for target_model in labels.keys():
        ...
        acc, avg_rank = compute_topk_accuracy_and_avg_rank(
            sorted_indices, gt_mapping, k
        )
...
def rec_loss_fn(ins, recons, logger=None, prefix=""):
    """
    Reconstruction loss for embedding-only setup.

    ins:    {domain: Tensor}
    recons: {domain: Tensor}
    """
    losses = []

    for domain, x in ins.items():
        if domain not in recons:
            continue
        ...
        if logger is not None:
            logger.logkv(f"{prefix}rec_{domain}_cos", cos_sim.mean().item())

    if len(losses) == 0:
        return torch.tensor(0.0, device=next(iter(ins.values())).device)
...
def validation_loop(
    ...
    rec_losses = []
    with torch.no_grad():
        for sup_batch, unsup_batch in zip(sup_valloader, unsup_valloader):
            ins = {
                **sup_batch,
                **unsup_batch,
            }

            recons, translations = translator(ins)

            rec_loss = rec_loss_fn(ins, recons)
    ...
    translator.train()
    return {
        "val/rec_loss": np.mean(rec_losses),
    }
